...
from Initialization import initialization_KNN
from weights import calc_weights

def KNNWOA(train,label,cv,P,Iter):
    size=train.shape
    dim=size[1]
    ...
    positions=initialization_KNN(lb,ub,P)
    Leader_score=1
    Leader_pos=[]
    CC=[]
    curr_iter=0
...
#        print(curr_iter)
        for i in range(0,P):
            ...
            if(sze[1]==0):
                ...
                fitness=1-KNNCrossValidation(weighted_train,label,cv,positions[i,dim])
