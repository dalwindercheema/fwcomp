...
from Initialization import initialization_NB
from weights import calc_weights

def NBWOA(train,label,cv,P,Iter):
    size=train.shape
    dim=size[1]
    lb=numpy.zeros([dim,1])
    ub=numpy.ones([dim,1])
    positions=initialization_NB(lb,ub,P)
    Leader_score=1
    Leader_pos=[]
    CC=[]
    curr_iter=0
...
#        print(curr_iter)
        for i in range(0,P):
            ...
            if(sze[1]==0):
                ...
                fitness=1-NBCrossValidation(weighted_train,label,cv)
