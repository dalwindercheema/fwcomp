...
from Initialization import initialization_CSVMR
...
def CRWOA(train,label,cv,P,Iter):
    ...
    positions=initialization_CSVMR(lb,ub,P)
    Leader_score=1
    Leader_pos=[]
    CC=[]
    curr_iter=0
...
#        print(curr_iter)
        for i in range(0,P):
            ...
            if(sze[1]==0):
                ...
                fitness=1-RBFSVMCrossValidation(weighted_train,label,cv,positions[i,dim],positions[i,dim+1])
